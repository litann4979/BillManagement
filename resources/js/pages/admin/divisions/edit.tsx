...
interface Props {
    division: { id: number; name: string; circle_id: number };
    circles: { id: number; name: string }[];
}

export default function Edit({ division, circles }: Props) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        name: division.name,
        circle_id: String(division.circle_id ?? ''),
    });
                        ...
                        <div className="flex items-center space-x-3 mb-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-500 dark:text-slate-400">
                                    Update division{' '}
                                    <span className="font-medium text-blue-600 dark:text-blue-400">{division.name}</span>
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200 dark:border-gray-700 overflow-hidden">
                            <div className="px-6 sm:px-8 pt-6 sm:pt-8 pb-4 bg-gradient-to-r from-blue-600/5 to-indigo-600/5 border-b border-slate-200 dark:border-gray-700">
                                ...
                                <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
                                    Edit the details of the division
                                </p>
                            </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                                            <div className="space-y-2">
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => setData('circle_id', e.target.value)}
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <Input
                                                    ...
                                                    className="h-11 rounded-xl border-2 border-slate-200 dark:border-gray-700 focus:border-blue-500 focus:ring-blue-500 placeholder:text-slate-400"
                                                    placeholder="e.g. Sambalpur Division"
                                                />
                                                <InputError
                                                    message={errors.name}
                                                    className="text-xs text-red-600 dark:text-red-400"
