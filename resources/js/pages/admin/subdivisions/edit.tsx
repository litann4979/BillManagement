...
interface Props {
    subdivision: {
        ...
        name: string;
        division_id: number;
        division?: { id: number; circle_id: number; circle?: { id: number } };
    };
    circles: { id: number; name: string }[];
}

export default function Edit({ subdivision, circles }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Dashboard', href: '/dashboard' },
        { title: 'Subdivisions', href: '/admin/subdivisions' },
        { title: subdivision.name, href: `/admin/subdivisions/${subdivision.id}/edit` },
    ];

    const { data, setData, put, processing, errors } = useForm({
        name: subdivision.name,
        division_id: String(subdivision.division_id ?? ''),
    });

    const div = subdivision.division;
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/subdivisions/${subdivision.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Edit Subdivision" />
                        ...
                        <div className="flex items-center space-x-3 mb-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-500 dark:text-slate-400">
                                    Update subdivision <span className="font-medium text-blue-600 dark:text-blue-400">{subdivision.name}</span>
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form onSubmit={submit} className="space-y-6 max-w-2xl">
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200 dark:border-gray-700 overflow-hidden">
                            <div className="px-6 sm:px-8 pt-6 sm:pt-8 pb-4 bg-gradient-to-r from-blue-600/5 to-indigo-600/5 border-b border-slate-200 dark:border-gray-700">
                                ...
                                <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">Edit subdivision name</p>
                            </div>
                            <div className="p-6 sm:p-8">
                                <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-4 flex items-center">
                                    <span className="w-1 h-4 bg-gradient-to-b from-blue-500 to-indigo-500 rounded-full mr-2"></span>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 sm:justify-end">
                            ...
                            >
                                {processing ? 'Updating...' : 'Update Subdivision'}
                            </Button>
                        </div>
                    </form>
                </div>
