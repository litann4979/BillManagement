...
interface Props {
    village: {
        ...
        name: string;
        section_id: number;
        section?: { id: number; subdivision_id: number; subdivision?: { id: number; division_id: number; division?: { id: number; circle_id: number } } };
    };
    circles: { id: number; name: string }[];
}

export default function Edit({ village, circles }: Props) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        name: village.name,
        section_id: String(village.section_id ?? ''),
    });

    const sec = village.section;
    const sub = sec?.subdivision;
    const div = sub?.division;
    const [hierarchy, setHierarchy] = useState({
        circle_id: String(div?.circle_id ?? ''),
        division_id: String(sub?.division_id ?? div?.id ?? ''),
        subdivision_id: String(sec?.subdivision_id ?? sub?.id ?? ''),
        section_id: String(village.section_id ?? ''),
        village_id: '',
        feeder_id: '',
        dtr_id: '',
    ...
    const handleHierarchy = (key: string, value: string) => {
        ...
        if (key === 'section_id') setData('section_id', value);
    };

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/villages/${village.id}`);
    };
                        ...
                        <div className="flex items-center space-x-3 mb-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-500 dark:text-slate-400">
                                    Update village <span className="font-medium text-blue-600 dark:text-blue-400">{village.name}</span>
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form onSubmit={submit} className="space-y-6">
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200 dark:border-gray-700 overflow-hidden">
                            <div className="px-6 sm:px-8 pt-6 sm:pt-8 pb-4 bg-gradient-to-r from-blue-600/5 to-indigo-600/5 border-b border-slate-200 dark:border-gray-700">
                                ...
                                <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1 ml-4">Select Circle, Division, Subdivision, and Section</p>
                            </div>
                            <div className="p-6 sm:p-8">
                                <HierarchyDropdowns values={hierarchy} onChange={handleHierarchy} circles={circles} showLevels={['circle_id', 'division_id', 'subdivision_id', 'section_id']} />
                                <InputError message={errors.section_id} className="mt-2 text-xs text-red-600 dark:text-red-400" />
                            </div>
                        </div>
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200 dark:border-gray-700 overflow-hidden">
                            <div className="px-6 sm:px-8 pt-6 sm:pt-8 pb-4 bg-gradient-to-r from-blue-600/5 to-indigo-600/5 border-b border-slate-200 dark:border-gray-700">
                                ...
                                <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1 ml-4">Edit village details</p>
                            </div>
                            <div className="p-6 sm:p-8">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 sm:justify-end">
                            ...
                            <Button
                                type="submit"
                                disabled={processing}
                                className="rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-medium shadow-lg shadow-blue-600/30 hover:shadow-xl hover:shadow-blue-600/40 transition-all duration-300 hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 px-6 py-2.5"
                            >
