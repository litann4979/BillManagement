...
interface FeederItem {
    ...
    name: string;
    voltage: string | null;
    section: { name: string; subdivision: { name: string } | null } | null;
    dtrs_count: number;
}

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Dashboard', href: '/dashboard' },
    { title: 'Feeders', href: '/admin/feeders' },
];

export default function Index({ feeders }: { feeders: FeederItem[] }) {
    const [searchTerm, setSearchTerm] = useState('');

    const handleDelete = (id: number) => {
        if (confirm('Delete this feeder?')) router.delete(`/admin/feeders/${id}`);
    };

    const filtered = feeders.filter(
        (f) =>
            f.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
            (f.section?.name ?? '').toLowerCase().includes(searchTerm.toLowerCase())
    );

    const totalDtrs = feeders.reduce((s, f) => s + f.dtrs_count, 0);
                    ...
                    <div className="mb-8">
                            ...
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-slate-500 dark:text-slate-400">
                                        Manage feeders in the hierarchy
                                    </p>
                                </div>
                            </div>
                            <Link
                                href="/admin/feeders/create"
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                            <div className="bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm rounded-xl p-4 border border-slate-200 dark:border-gray-700">
                                <p className="text-2xl font-bold text-slate-900 dark:text-white">{feeders.length}</p>
                                <p className="text-xs text-slate-500 dark:text-slate-400">Total Feeders</p>
                            </div>
                            <div className="bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm rounded-xl p-4 border border-slate-200 dark:border-gray-700">
                                <p className="text-2xl font-bold text-slate-900 dark:text-white">{totalDtrs}</p>
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200 dark:border-gray-700 overflow-hidden">
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-900 dark:to-gray-800">
                                            ...
                                            <th className="p-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Voltage</th>
                                            <th className="p-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">DTRs</th>
                                    ...
                                    <tbody className="divide-y divide-slate-200 dark:divide-gray-700">
                                        {filtered.map((f) => (
                                            <tr key={f.id} className="group hover:bg-blue-50/50 dark:hover:bg-gray-700/50 transition-all duration-200">
                                                <td className="p-4">
                                                    <div className="flex items-center space-x-3">
                                                ...
                                                <td className="p-4 text-slate-500 dark:text-slate-400">{f.section?.name ?? '—'}</td>
                                                <td className="p-4 text-slate-500 dark:text-slate-400">{f.voltage ?? '—'}</td>
                                                <td className="p-4">
                                                ...
                                                <td className="p-4 text-right">
                                                    <div className="flex items-center justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                                        <Link href={`/admin/feeders/${f.id}/edit`} className="p-2 text-blue-600 hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-gray-700 rounded-lg transition-colors" title="Edit">
                                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                            </svg>
                                                        </Link>
                                                        <button onClick={() => handleDelete(f.id)} className="p-2 text-red-500 hover:bg-red-50 dark:text-red-400 dark:hover:bg-gray-700 rounded-lg transition-colors" title="Delete">
                                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        {filtered.length === 0 && (
                                            <tr>
                                                <td colSpan={5} className="p-12 text-center">
                                                    <p className="text-slate-500 dark:text-slate-400 font-medium">No feeders found</p>
                            ...
                            <div className="px-6 py-4 border-t border-slate-200 dark:border-gray-700">
                                <p className="text-sm text-slate-500 dark:text-slate-400">
                                    ...
                                    <span className="font-medium">{feeders.length}</span> feeders
                                </p>
                            </div>
                        </div>
                    </div>
