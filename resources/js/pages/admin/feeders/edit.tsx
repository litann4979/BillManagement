    ...
    feeder: {
        ...
        section?: {
            ...
            subdivision?: {
                id: number;
                division_id: number;
                division?: { id: number; circle_id: number };
            };
        };
    };
...
export default function Edit({ feeder, circles }: Props) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        name: feeder.name,
        section_id: String(feeder.section_id ?? ''),
        voltage: feeder.voltage ?? '',
    });

    const sec = feeder.section;
    const sub = sec?.subdivision;
    const div = sub?.division;
    const [hierarchy, setHierarchy] = useState({
        circle_id: String(div?.circle_id ?? ''),
        division_id: String(sub?.division_id ?? ''),
        subdivision_id: String(sec?.subdivision_id ?? ''),
        section_id: String(feeder.section_id ?? ''),
        village_id: '',
        feeder_id: '',
        dtr_id: '',
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/feeders/${feeder.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Edit Feeder" />
                        ...
                        <div className="flex items-center space-x-3 mb-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-500 dark:text-slate-400">
                                    Update feeder <span className="font-medium text-blue-600 dark:text-blue-400">{feeder.name}</span>
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form onSubmit={submit} className="space-y-6">
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200 dark:border-gray-700 overflow-hidden">
                            <div className="px-6 sm:px-8 pt-6 sm:pt-8 pb-4 bg-gradient-to-r from-blue-600/5 to-indigo-600/5 border-b border-slate-200 dark:border-gray-700">
                                ...
                                <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">Select the section for this feeder</p>
                            </div>
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200 dark:border-gray-700 overflow-hidden">
                            <div className="px-6 sm:px-8 pt-6 sm:pt-8 pb-4 bg-gradient-to-r from-blue-600/5 to-indigo-600/5 border-b border-slate-200 dark:border-gray-700">
                                ...
                                <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">Feeder name and voltage</p>
                            </div>
                            <div className="p-6 sm:p-8">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="voltage" className="text-sm font-medium text-slate-700 dark:text-slate-300">
                                            Voltage
                                        </Label>
                                        <Input
                                            id="voltage"
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 sm:justify-end">
                            ...
                            >
                                {processing ? 'Updating...' : 'Update Feeder'}
                            </Button>
                        </div>
                    </form>
                </div>
